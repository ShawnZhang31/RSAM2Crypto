...
@implementation RSAM2

#pragma mark - 将NSData转化为Base64n编码的字符串
...
#pragma mark - 将Base64编码的字符串转换为NSData
...
{
    if (!str || !pub_str) return nil;
    return [self encryptString:str publicKey:pub_str];
}

#pragma mark - 使用'.p12'私钥文件签名
...
{
    if (!str || !path) return nil;
    NSData* signedData= [self signatureData:[str dataUsingEncoding:NSUTF8StringEncoding] andPrivateKeyRef:[self getPrivateKeyRefWithContentsOfFile:path password:password] withAlgo:algorithm];
...
+ (NSString *)encryptString:(NSString *)str privateWithContentsOfFile:(NSString *)path password:(NSString *)password
{
    ...
    return base64_encode_data([self encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] withKeyRef:keyRef isSign:YES]);
}

#pragma mark - 使用私钥字符串加密
+ (NSString *)encryptString:(NSString *)str privateKeyString:(NSString *)pri_str
{
    if (!str || !pri_str) return nil;
    
    SecKeyRef keyRef = [self addPKCS8PrivateKey:pri_str];
    if (!keyRef) {
        return nil;
    }
...
+ (NSString *)decryptString:(NSString *)str publicKeyWithContentsOfFile:(NSString *)path
{
    ...
    return [[NSString alloc] initWithData:[self decryptData:data withKeyRef:keyRef] encoding:NSUTF8StringEncoding];
}
...
+ (NSString *)decryptString:(NSString *)str publicKeyString:(NSString *)pub_str
{
    if (!str || !pub_str) return nil;
    NSData *data = base64_decode(str);
    data = [self decryptData:data publicKey:pub_str];
...
{
    if (!str || !pri_str) return nil;
    return [self decryptString:str privateKey:pri_str];
}

//#pragma mark - 使用'.12'私钥文件加密
//+(NSString *)encryptString:(NSString *)str privateKeySelf:(SecKeyRef)privateKeyRef{
...
+(NSString *)signString:(NSString *)str privateKeyRef:(SecKeyRef)privateKeyRef withAlgo:(SecKeyAlgorithm)algorithm{
    if(![str dataUsingEncoding:NSUTF8StringEncoding]){
        return nil;
    }
    if(!privateKeyRef){
...
//    NSData *data = [self encryptData:[str dataUsingEncoding:NSUTF8StringEncoding] withKeyRef:privateKeyRef];
    NSData *data = [self signatureData:[str dataUsingEncoding:NSUTF8StringEncoding] andPrivateKeyRef:privateKeyRef withAlgo:algorithm];
    NSString *ret = base64_encode_data(data);
    return ret;
}
...
+(NSData *)signatureData:(NSData*)data andPrivateKeyRef:(SecKeyRef)keyRef withAlgo:(SecKeyAlgorithm)algorithm{
    NSData* signature = nil;
    CFErrorRef error = NULL;
    signature = (NSData*)CFBridgingRelease(
                SecKeyCreateSignature(keyRef,
                                      algorithm,
    ...
    if (!signature)
    {
        ...
        NSLog(@"privatekey signature error:%@", err.localizedDescription);
    }
    return signature;
}

+(BOOL) verifyData:(NSData *)origin andSignatureData:(NSData *)signature andKeyRef:(SecKeyRef)publicKeyRef withAlgp:(SecKeyAlgorithm)alpgorithm
{
    
    BOOL result = NO;
    CFErrorRef error =NULL;
    result = SecKeyVerifySignature(publicKeyRef,
                                   alpgorithm,
                                   (__bridge CFDataRef)origin,
...
//    SecKeyRef keyRef = [self addPrivateKey:privKey];
    SecKeyRef keyRef = [self addPKCS8PrivateKey:privKey];
    if(!keyRef){
        return nil;
    }
    return [self decryptData:data withKeyRef:keyRef];
...
#pragma mark -m 从PKCS8格式的字符串中h获取私钥
+ (SecKeyRef)addPKCS8PrivateKey:(NSString *)key{
    NSRange spos;
    NSRange epos;
